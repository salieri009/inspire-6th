...
// [학습 정리] 변수의 종류/스코프 + Wrapper 클래스 + 형변환(casting)
// - 기본타입(값 저장) vs 참조타입(주소 저장), 멤버변수 vs 지역변수(선언 위치로 구분)
// - Integer(Wrapper) <-> int(primitive) 사이의 오토박싱/언박싱
// - byte + byte 연산 결과는 int로 자동 승격되므로 다시 byte에 담으려면 명시적 캐스팅이 필요
