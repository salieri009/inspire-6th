...
// [학습 정리] 상속(inheritance)과 다형성(polymorphism)
// - StudentDTO/TeacherDTO/ManagerDTO 는 PersonDTO를 상속(extends), super() 로 부모 생성자 호출
// - 변수타입의 다형성: PersonDTO 타입 변수에 자식 객체를 담을 수 있음(업캐스팅은 암묵적)
// - 다운캐스팅: 자식 고유 필드/메서드에 접근하려면 (ManagerDTO)manager 처럼 명시적 캐스팅 필요
// - 매개변수의 다형성: OopService.makePerson(Flag, ...) 처럼 enum 플래그로 어떤 하위타입을 만들지 분기
import backend.lecture0818.features.oop.service.OopService;
import backend.lecture0818.features.oop.sub.ManagerDTO;
import backend.lecture0818.features.oop.sub.StudentDTO;
import backend.lecture0818.features.oop.sub.TeacherDTO;
import backend.lecture0818.features.oop.sup.PersonDTO;
