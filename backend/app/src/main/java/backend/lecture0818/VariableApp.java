...
// [학습 정리] 변수와 타입
// - 기본타입(primitive): 값 자체를 저장 / 참조타입(reference): 주소값을 저장(new로 생성)
// - 변수의 스코프: 클래스 블록에 선언하면 멤버변수, 메서드 블록에 선언하면 지역변수
// - Wrapper Class(Boxing/UnBoxing): int <-> Integer 자동 변환
// - 형변환(casting): byte -> short -> int -> long -> float -> double 순으로 자동 확장, 반대는 명시적 캐스팅 필요
