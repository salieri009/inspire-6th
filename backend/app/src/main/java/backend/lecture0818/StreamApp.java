...
// [학습 정리] 함수형 인터페이스(Functional Interface)와 Stream API
// - 커스텀 함수형 인터페이스(InspireFunction)에 람다식 대입, 메서드 1개만 정의된 인터페이스여야 함
// - 표준 함수형 인터페이스: Supplier(공급, 매개변수 X), Consumer(소비, 반환값 X), Function(입력->출력), Predicate(조건->boolean)
// - Consumer.andThen 으로 소비 동작을 체이닝
// - Stream: 원본 컬렉션을 변경하지 않고 filter/map/collect 로 가공, forEach로 최종 소비, 메서드 참조(System.out::println)
...
import backend.lecture0818.features.lambda.InspireFunction;
