...
// [학습 정리] 제네릭(Generics) 와일드카드
// - <? extends T> : T 및 그 하위타입만 허용, 읽기전용(get은 가능, add는 사실상 불가)
// - <? super T>   : T 및 그 상위타입만 허용, 쓰기전용(add는 가능, get은 Object로만 안전)
// - 커스텀 제네릭 클래스(ResponseTemplate<T>)로 응답 코드/메시지/데이터를 타입 안전하게 래핑
import java.util.ArrayList;
import java.util.List;

import backend.lecture0818.features.generics.ResponseTemplate;
